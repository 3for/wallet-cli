...
public class WalletApi {
  ...
  private static boolean isSnarkInited = librustzcashInitZksnarkParams();
  ...
  private void showTransactionAfterSign(Transaction transaction)
    ...
    if (transaction.getRawData().getContract(0).getType() == ContractType.CreateSmartContract) {
      ...
      System.out.println("zyd smart contract address will be:"+ ByteArray.toHexString(contractAddress));
      System.out.println(
  ...
  private static void checkShieldedTRC20NoteValue(
    ...
    if (!Objects.isNull(spendNoteTRC20s)) {
      for (GrpcAPI.SpendNoteTRC20 spendNote : spendNoteTRC20s) {
        if (spendNote.getNote().getValue() < 0) {
          System.out.println("The value in SpendNoteTRC20 must >= 0");
    ...
    if (!Objects.isNull(receiveNotes)) {
      for (ReceiveNote receiveNote : receiveNotes) {
        if (receiveNote.getNote().getValue() < 0) {
          System.out.println("The value in ReceiveNote must >= 0");
  ...
  private static void buildShieldedTRC20Output(ShieldedTRC20ParametersBuilder builder,
    ...
    if (Objects.isNull(paymentAddress)) {
      System.out.println(PAYMENT_ADDRESS_FORMAT_WRONG);
  ...
  private static void buildShieldedTRC20Input(ShieldedTRC20ParametersBuilder builder,
  ...
  private static byte[] triggerGetScalingFactor(byte[] contractAddress) {
    ...
    GrpcAPI.TransactionExtention trxExt2 = rpcCli.triggerConstantContract(
  ...
  private static void checkBigIntegerRange(BigInteger in) throws ContractValidateException {
    if (in.compareTo(BigInteger.ZERO) < 0) {
      System.out.println("public amount must be non-negative");
      throw new ContractValidateException("public amount must be non-negative");
    }
    if (in.bitLength() > 256) {
      System.out.println("public amount must be no more than 256 bits");
      throw new ContractValidateException("public amount must be no more than 256 bits");
    }
  }

  private static BigInteger getBigIntegerFromString(String in) {
   ...
   */
  private static long[] checkPublicAmount(byte[] address, BigInteger fromAmount, BigInteger toAmount)
    ...
    if (!(fromAmount.mod(scalingFactor).equals(BigInteger.ZERO)
      ...
      System.out.println("fromAmount or toAmount invalid");
  ...
  public static ShieldedTRC20Parameters zydClientCreateShieldedContractParameters(
      PrivateShieldedTRC20Parameters request) throws ContractValidateException, ZksnarkException, ContractExeException {
    System.out.println("zyd client here");
    ShieldedTRC20ParametersBuilder builder = new ShieldedTRC20ParametersBuilder();

    System.out.println("zyd 1");
    ...
    if (ArrayUtils.isEmpty(shieldedTRC20ContractAddress)
      ...
      System.out.println("No valid shielded TRC-20 contract address");
      throw new ContractValidateException("No valid shielded TRC-20 contract address");
    }

    System.out.println("zyd 2");
    ...
    try {
      ...
      System.out.println("invalid from_amount or to_amount");
      throw new ContractValidateException("invalid from_amount or to_amount");
    }

    System.out.println("zyd 4");
    ...
    if (scaledToAmount > 0) {
      try {
        ...
        System.out.println("Unbalanced burn!");
        throw new ZksnarkException("Unbalanced burn!");
      }
    }

    System.out.println("zyd 7");
    if (scaledFromAmount > 0 && spendSize == 0 && receiveSize == 1
      ...
      if ((ArrayUtils.isEmpty(ask) || ArrayUtils.isEmpty(nsk) || ArrayUtils.isEmpty(ovk))) {
        System.out.println("No shielded TRC-20 ask, nsk or ovk");
      ...
      if (ArrayUtils.isEmpty(transparentToAddress) || transparentToAddress.length != 21) {
        System.out.println("No valid transparent TRC-20 output address");
      ...
      System.out.println("invalid shielded TRC-20 parameters");
      throw new ContractValidateException("invalid shielded TRC-20 parameters");
    }
    System.out.println("zyd 11");
